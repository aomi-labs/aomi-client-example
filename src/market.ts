...
async function fetchTokenPrice(
  ...
  if (!res.ok)
    throw new Error(`GeckoTerminal token price error: ${res.status}`);
  const data = await res.json();
  const price = parseFloat(
    data?.data?.attributes?.token_prices?.[address.toLowerCase()] ?? "0",
  );
  if (!price || isNaN(price))
    throw new Error(`No token price returned for ${address}`);
  return price;
}
...
export async function fetchMarketData(config: BotConfig): Promise<MarketData> {
  ...
  const pricePromise = fetchTokenPrice(
    config.geckoNetwork,
    config.riskAssetAddress,
  );
  const poolStatsPromise = fetchPoolStats(
    config.geckoNetwork,
  ...
  const now = Date.now();
  if (
    now - lastOhlcvFetchAt > config.ohlcvRefreshMs ||
    ...
    try {
      ...
      console.warn(
        "[market] OHLCV fetch failed, using cached:",
