...
> Example trading bot showing how to integrate `@aomi-labs/client` — the TypeScript SDK for Aomi, open-source AI infrastructure for automating crypto.

## What is this?

This repository is a reference implementation built on `@aomi-labs/client`, the TypeScript SDK for Aomi — open-source AI infrastructure for automating crypto. The bot rotates between a risk asset and a stable asset using moving-average signals, sending natural-language trade instructions to the Aomi backend and auto-signing the resulting on-chain transactions and EIP-712 payloads locally.
